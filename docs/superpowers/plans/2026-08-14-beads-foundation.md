...
epic_id = "${epic_id}"
children = {
    "Cancel member resolves physical LO queue identity": ("D13, D23", "Phase 4"),
    "Auto-video blocks queued and running duplicates": ("D07, D12", "Phase 3"),
    "Auto producer obeys the retry stop-generation barrier": ("D15, D23", "Phase 8"),
    "Multi-copy enqueue assigns distinct job identities": ("D01, D03", "Phase 3"),
    "Forced-account constraint survives every retry item": ("D18", "Phase 5"),
}
...
for issue in selected:
    decision, phase = children[issue["title"]]
    ...
    assert all(needle in issue["acceptance_criteria"] for needle in ("Remove only this @unittest.expectedFailure decorator", "observe red before implementation", "same targeted test is green", "full lifecycle gate is green", "the current xfailed baseline decreases by exactly one"))
PY
