...
for s in d['scenes']:
    for f in s.get('sfs', []):
        ...
        for t in trong_khung:
            # Need to match t in the REF ID. For example, t='MAYA' -> REF_MAYA_...
            # Ref IDs are like REF_MAYA_PORTRAIT or REF_MAYA_DEN_FULL.
            # So `t` should be surrounded by _ or at the end of REF_
            # We can use regex or simple string match: f"_{t}_" or f"_{t}"
            t_upper = t.upper()
            co_full = any(re.search(r'_' + re.escape(t_upper) + r'(_|$)', c) for c in f.get('refs',{}).get('chars',[]) if c.endswith('_FULL'))
            if not co_full:
